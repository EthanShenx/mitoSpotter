#!/usr/bin/env python3
# Decode sequences using the 3-nt (non-overlap triplet) HMM. Species REQUIRED.
...
def clean_nt(s):
    return "".join([c for c in str(s).upper().replace("U","T") if c in BASES])
...
def resolve_assets(species, assets_dir):
    model = op.join(assets_dir, f"{species}_mitoSpotter_hmm_3nt.json")
    vocab = op.join(assets_dir, f"{species}_nt3_vocab.json")
...
def load_model(p):
    ...
    order = M.get("triplet_order")
    idx = {c:i for i,c in enumerate(order)} if order else {c:i for i,c in enumerate([a+b+c for a in "ACGT" for b in "ACGT" for c in "ACGT"])}
    return pi,A,B,idx

def load_states(p):
    S=json.load(open(p)); S={int(k):v for k,v in S.items()}; inv={v:k for k,v in S.items()}
...
def forward_ll(pi,A,B,obs):
    ...
    for t in range(1,T):
        alpha[t]=(alpha[t-1]@A)*B[:,obs[t]]; c[t]=alpha[t].sum() or 1e-300; alpha[t]/=c[t]
...
def iter_inputs(args):
    ...
    if args.fasta:
        for rec in SeqIO.parse(args.fasta, "fasta"): yield rec.id, str(rec.seq)

def main():
    ap=argparse.ArgumentParser(description="3-nt (non-overlap triplet) Viterbi decoding. Species REQUIRED.")
    ...
    ap.add_argument("--segment", choices=["cds","orf"], default="cds")
    ...
    with open(args.out_tsv, "w") as fo:
        ...
        for rid, raw in iter_inputs(args):
            ...
            if args.segment=="cds":
                ...
                tokens = to_triplets_nonoverlap(s)
                ...
                cands.append((ll,vll,path,obs.size,"cds"))
            else:
                for code in (["standard","vertebrate_mito"] if args.code=="auto" else [args.code]):
                    ...
                    orf=find_longest_orf_nt(raw, stops, min_nt=args.min_len)
                    if not orf: continue
                    tokens = to_triplets_nonoverlap(orf)
                    obs = np.array([idx[t] for t in tokens if t in idx], dtype=np.int64)
                    if obs.size==0: continue
                    ll=forward_ll(pi,A,B,obs); vll,path=viterbi(pi,A,B,obs)
                    cands.append((ll,vll,path,obs.size,code))
