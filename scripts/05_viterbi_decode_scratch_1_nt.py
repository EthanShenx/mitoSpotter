#!/usr/bin/env python3
# Viterbi decode with 1-nt HMM. Requires --species; accepts FASTA/--seq/--stdin.
from Bio import SeqIO
import argparse, os.path as op, json, re, sys, numpy as np

STD_STOPS={"TAA","TAG","TGA"}
VERTEBRATE_MITO_STOPS={"TAA","TAG","AGA","AGG"}
BASES=["A","C","G","T"]; IDX={b:i for i,b in enumerate(BASES)}
...
def main():
    ap=argparse.ArgumentParser(description="1-nt Viterbi decoding. Species REQUIRED.")
    ...
    ap.add_argument("--code", choices=["standard","vertebrate_mito","auto"], default="auto", help="Only meaningful if --segment orf.")
    ...
    pi,A,B=load_model(model_p); state_names,nuc_id,mito_id=load_states(states_p)

    with open(args.out_tsv,"w") as fo:
        fo.write("#id\tlogprob\twinner\tnuclear_frac\tmito_frac\tlen_nt\n")
        for rid, raw in iter_inputs(args):
            ...
            if args.segment=="cds":
                ...
                cands.append((ll,vll,path,len(obs),"cds"))
            else:
                for code in (["standard","vertebrate_mito"] if args.code=="auto" else [args.code]):
                    ...
                    if not orf: continue
                    obs=np.array([IDX[c] for c in orf if c in IDX], dtype=np.int64)
                    if obs.size==0: continue
                    ll=forward_ll(pi,A,B,obs); vll,path=viterbi(pi,A,B,obs)
                    cands.append((ll,vll,path,len(obs),code))
