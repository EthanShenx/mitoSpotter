...
def main():
    ...
    ap.add_argument("--protein_coding_only", action="store_true", default=True,
                    help="Keep only protein_coding transcripts (default: True). Use --no-protein_coding_only to disable.")
    ap.add_argument("--no-protein_coding_only", dest="protein_coding_only", action="store_false")
    args = ap.parse_args()
    ...
    with open(os.path.join(args.outdir, "ids_mito.txt"), "w") as fo:
        fo.write("\n".join(sorted(mito_ids)) + ("\n" if mito_ids else ""))
    with open(os.path.join(args.outdir, "ids_nuclear.txt"), "w") as fo:
        fo.write("\n".join(sorted(nuc_ids)) + ("\n" if nuc_ids else ""))

    print(f"[OK] mito IDs: {len(mito_ids)} -> out/ids_mito.txt")
    print(f"[OK] nuclear IDs: {len(nuc_ids)} -> out/ids_nuclear.txt")
